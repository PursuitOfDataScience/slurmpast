...
``nodes``, ``index``, ``sizing``, ``logs``, ``site``, ``cache`` and ``duration``
-- import no third-party package, so they are usable as a library on a login node
without a UI framework. ``rich`` and ``textual`` are reached for only by
``render``, ``report``, ``theme`` and ``tui``, which is what
...
the sentence promising it was short. ``cache`` joined them later, and the same
test caught the omission the same way.
