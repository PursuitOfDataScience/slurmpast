...
def history() -> list:
    ...
    for index in range(20):
        ...
        rows += _job(jid, "cot-exp", state, 1826, 30, cpu,
                     mem="80G", rss="2000000K", cpus=6, gpus=1,
                     node="midway3-0385" if index % 3 else "midway3-0602", day=index + 1)

    # Healthy training runs -- these must draw no findings.
    for index in range(14):
        jid += 1
        rows += _job(jid, "midtrain", "COMPLETED", 6769, 120, "05:31:39",
                     mem="200G", rss="193517740K", cpus=4, gpus=3,
                     node="midway3-0600", day=index + 5,
                     read=112710599158, write=139565725797, system_cpu="15:35.996")

    # A --mem hand-search that ends by succeeding at a value that already OOM'd.
    for mem, state in (("48G", "CANCELLED by 940740146"), ("32G", "OUT_OF_MEMORY"),
                       ("32G", "OUT_OF_MEMORY"), ("17G", "OUT_OF_MEMORY"),
                       ("12G", "OUT_OF_MEMORY"), ("12G", "OUT_OF_MEMORY"),
                       ("14G", "OUT_OF_MEMORY"), ("16G", "OUT_OF_MEMORY"),
                       ("18G", "OUT_OF_MEMORY"), ("32G", "COMPLETED")):
        jid += 1
        rows += _job(jid, "rc-tok-github_code", state, 1200, 480, "00:19:00",
                     mem=mem, rss="%dK" % (int(mem[:-1]) * 1024 * 1024 + 500000),
                     cpus=16, node="midway3-0432", day=20,
                     read=67022736995, exit_code="0:125")

    # A parameter sweep -- exercises the name-pattern rollup (att-speed-#).
    for step in (11, 17, 23, 28, 34, 40, 51):
        jid += 1
        rows += _job(jid, "att-speed-%d" % step, "COMPLETED", 2400, 60, "05:00:00",
                     mem="96G", rss="40000000K", cpus=12, gpus=1,
                     node="midway3-0606", day=12)

    # Kernel-heavy: mostly system time, the signal nothing else reports.
    for index in range(4):
        jid += 1
        rows += _job(jid, "tokenize-shards", "COMPLETED", 3600, 120, "02:00:00",
                     mem="32G", rss="12000000K", cpus=16, node="midway3-0601",
                     day=index + 2, system_cpu="01:20:00",
                     read=340 * 1024 ** 3, write=90 * 1024 ** 3)

    # A multi-task job with one lagging rank.
    jid += 1
    rows += _job(jid, "ddp-pretrain", "FAILED", 5400, 180, "08:00:00",
                 mem="160G", rss="90000000K", cpus=32, gpus=4, ntasks=8,
                 node="midway3-0372", day=22, exit_code="1:0",
                 ave_cpu="01:00:00", min_cpu="00:12:00")

    # A long allocation that held GPUs and never computed.
    jid += 1
    rows += _job(jid, "node-evaluation", "CANCELLED by 940740146", 151200, 2160,
                 "00:00.540", mem="64G", rss="1956K", cpus=8, gpus=2,
                 node="midway3-0385", day=26)

    # A job that paged heavily.
    jid += 1
    rows += _job(jid, "soup-merge", "COMPLETED", 4200, 120, "01:05:00",
                 mem="48G", rss="47000000K", cpus=8, node="midway3-0605",
                 day=9, pages=820000)
